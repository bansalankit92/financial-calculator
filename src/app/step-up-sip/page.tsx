...
export default function StepUpSIPCalculatorPage() {
  ...
  const initialInvestment = getQueryParam(
    'investment',
    getDefaultStepUpInvestment(stepUpFrequency)
  );
  const stepUpValue = getQueryParam(
    'stepup',
    getDefaultStepUpValue(stepUpFrequency, stepUpType)
  );
  const interestRate = getQueryParam('interest', 12);
  const years = getQueryParam('years', 10);
  ...
  const updateQueryParams = (params: { [key: string]: string | number }) => {
  ...
  const handleStepUpValueChange = (value: number) => {
    updateQueryParams({
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <StepUpForm
          ...
          onStepUpValueChange={handleStepUpValueChange}
